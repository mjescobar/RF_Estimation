...
def main():
	...
	parser = argparse.ArgumentParser(prog='clusteringAmplitudePeak.py',
	...
	amplitudePeak = zeros((data.shape[0],2))
	for unitId in range(data.shape[0]):
		x = linspace(1, framesNumber, framesNumber)
		s = UnivariateSpline(x, data[unitId,0:framesNumber], s=0)
		...
		peak = unique(where(peakTempCurve==ys)[0])[0]
		minimo = amin(data[unitId])
		maximo = amax(data[unitId])
