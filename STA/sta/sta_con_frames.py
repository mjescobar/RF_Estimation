...
def calculaSTA(args):
	...
	for kunit in range(start,finish):
		...
		if characterization[kunit] > 0:
			...
			ax1.pcolormesh( component,vmin = 0,vmax = 255, cmap=cm.jet )
			...
			plt.savefig(finalfolder_lin+"/STA-"+str(neurontag)+"1"+"_.pdf",format='pdf', bbox_inches='tight')
			plt.show()
			...
			for ksubplot in range(framesNumber-1):
				...
				kcontador+=1
				plt.savefig(finalfolder_lin+"/STA-"+str(neurontag)+str(kcontador)+"_.pdf",format='pdf', bbox_inches='tight')
